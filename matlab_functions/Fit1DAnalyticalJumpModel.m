function [lsqparams, mleparams, mlecis] = Fit1DAnalyticalJumpModel(results, ...
...
%% first use LSQ fitting to estimate parameters:
    myfunLSQ = @(p,xdata) AnalyticalJumpModelWithNoise(xdata, sig1^2, ...
        sig2^2, f1, srange, 1/p(1), 1/p(2),t);
...
%% Now do MLE fitting:
    ...
    myfunMLE = @(x,b,mu) AnalyticalJumpModelWithNoise(x, sig1^2, sig2^2,...
        f1, srange, 1/b, 1/mu, t);
    ...
    bestfit= AnalyticalJumpModelWithNoise(x1, sig1^2, sig2^2, f1, srange,...
        1/phat(1), 1/phat(2), t);
